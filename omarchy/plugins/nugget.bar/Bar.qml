...
Item {
  ...
  // The omarchy-shell host injects omarchyPath from OMARCHY_PATH. Declared
  // optional (not `required`) because the host loads plugin bars via URL and
  // fills these properties in `onLoaded`, after instantiation.
  ...
  component BarPanel: PanelWindow {
    ...
    margins {
      top: root.barHidden && root.position === "top" ? -(root.barSize + 4) : (root.position === "top" ? 4 : 0)
      bottom: root.barHidden && root.position === "bottom" ? -(root.barSize + 4) : (root.position === "bottom" ? 4 : 0)
      left: root.barHidden && root.position === "left" ? -(root.barSize + 4) : (root.position === "left" ? 4 : 0)
      right: root.barHidden && root.position === "right" ? -(root.barSize + 4) : (root.position === "right" ? 4 : 0)
    }
